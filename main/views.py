...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if customer.is_valid():
            ...
            newuser = Customer()
            newuser.user = user
            newuser.firs_tname = user.first_name
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if passupdate:
            ...
            messages.error(request,f'the following errors occurred: {passupdate.errors}')
...
def add_to_cart(request):
    if request.method == 'POST':
        ...
        if cart:
            basket = Cart.objects.filter(user__username = request.user.username, product=product.id, price=product.price,paid=False)
            if basket:
...
def payment(request):
    if request.method =='POST':
        ...
        api_key = 'sk_test_c0ffadbf38892df3c7e97b29069721fcaa59a367' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://44.212.71.24/thankyou' #thank you page
